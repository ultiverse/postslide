...
export const useProject = create((set) => ({
    ...
    updateProjectTitle: (title) => set((s) => ({ project: { ...s.project, title } })),
    // Slide-level
    ...
    moveSlideDown: (id) => set((s) => {
        const idx = s.project.slides.findIndex((sl) => sl.id === id);
        if (idx === -1 || idx >= s.project.slides.length - 1)
            return s;
        const slides = [...s.project.slides];
    ...
    // Block-level
    addBlock: (slideId, kind) => set((s) => ({
        project: {
            ...s.project,
            slides: s.project.slides.map((sl) => sl.id === slideId
                ? {
                    ...sl,
                    blocks: [
                        ...sl.blocks,
    ...
    updateBlock: (slideId, blockId, text) => set((s) => ({
        project: {
            ...s.project,
            slides: s.project.slides.map((sl) => sl.id === slideId
                ? {
                    ...sl,
                    blocks: sl.blocks.map((b) => b.id === blockId && 'text' in b ? { ...b, text } : b),
                }
                : sl),
        },
    })),
    updateBullets: (slideId, blockId, bullets) => set((s) => ({
        project: {
            ...s.project,
            slides: s.project.slides.map((sl) => sl.id === slideId
                ? {
                    ...sl,
                    blocks: sl.blocks.map((b) => b.id === blockId && b.kind === 'bullets' ? { ...b, bullets } : b),
                }
                : sl),
        },
    })),
    ...
    moveBlockUp: (slideId, blockId) => set((s) => ({
        project: {
            ...s.project,
    ...
    moveBlockDown: (slideId, blockId) => set((s) => ({
        project: {
            ...s.project,
            slides: s.project.slides.map((sl) => {
                if (sl.id !== slideId)
                    return sl;
                const idx = sl.blocks.findIndex((b) => b.id === blockId);
                if (idx === -1 || idx >= sl.blocks.length - 1)
                    return sl;
                const blocks = [...sl.blocks];
    ...
    convertBlockKind: (slideId, blockId, newKind) => set((s) => ({
        project: {
            ...s.project,
            slides: s.project.slides.map((sl) => {
                if (sl.id !== slideId)
